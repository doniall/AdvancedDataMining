...
Colours in the overlay:
    ...
    red    = clutter, showing because it EXCEEDS ITS OWN BASELINE
             (no real rain needs to be nearby for this one -- if this lights
             up with zero real rain anywhere on the frame, the fake signal
             itself drifted above its own historical "usual" reading)
...
def main():
    ...
    exceeds = clutter & above_baseline
    ...
    leaking = clutter & (exceeds | touching | surrounded)
    print(f"{int(leaking.sum())} clutter pixels are showing (not suppressed) in this frame\n")

    ys, xs = np.where(leaking)
    for y, x in list(zip(ys, xs))[:30]:
        reasons = []
        if exceeds[y, x]:
            reasons.append("EXCEEDS_BASELINE")
        if touching[y, x]:
            reasons.append("touching")
        if surrounded[y, x]:
            reasons.append("surrounded")
        print(f"  (x={x}, y={y}): level={full_lvl[y, x]}  baseline={baseline[y, x]}  -> {', '.join(reasons)}")
    if leaking.sum() > 30:
